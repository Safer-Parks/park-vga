...
#SBATCH --mem=8G
#SBATCH --output=logs/test_height_parks3_%a.log
#SBATCH --error=logs/test_height_parks3_%a.err
#SBATCH --mail-type=END,FAIL
#SBATCH --account=proj_a

mkdir -p logs
eval "$(pixi shell-hook --shell=bash)"
python england_run_3.py \
    "$SLURM_ARRAY_TASK_ID" \
    "testing_leeds_bradford.csv" \
    "parks_gardens_id/" \
    "/mnt/scratch/earmmu/testing_height_aire_run_output_3/" \
